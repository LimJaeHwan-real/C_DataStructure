...
`C_DataStructure_Test` 폴더를 실제 문제 소스 폴더와 같은 루트 경로에 두면 바로 테스트를 실행할 수 있습니다.
...
2. `C_DataStructure_Test/` 폴더를 실제 문제 소스 폴더와 같은 루트에 둡니다.
3. 아래 명령을 한 번 실행합니다.
...
- `./test stack 3` 의 `3`은 Stack and Queue 3번 문제입니다.

폴더 이름은 꼭 `Data-Structures`일 필요가 없습니다. 같은 루트에 있는 폴더 중 아래 구조를 가진 폴더를 자동으로 찾아 사용합니다.
- `Linked_List/`
- `Stack_and_Queue/`
- `Binary_Tree/`
- `Binary_Search_Tree/`
...
공통 전제:
- `C_DataStructure_Test/` 폴더와 실제 문제 소스 폴더가 같은 루트에 있어야 합니다.
- 설치는 프로젝트 루트에서 실행합니다.
...
- 이 테스트는 실제 소스 파일 `<소스 루트>/Linked_List/Q5_A_LL.c`를 컴파일해서 검사합니다.
